...
function byIngredient(drinkTxt){
  var requestUrl = 'https://www.thecocktaildb.com/api/json/v1/1/filter.php?i='+drinkTxt;
  fetch(requestUrl)
...
function byDrinkID(drinkTxt){
  ...
  var requestUrl = 'https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i='+drinkTxt;
...
function selectionList(data, titleText){
  ...
  for(var i=0; i<data.drinks.length; i++){
    ...
    // $('#modallist', '#button').attr('id', 'value');
      ...
      $("#select-"+[i]).text(data.drinks[i].strDrink);
        }
  ...
  // $('#current-city').text(selectedCity);
    ...
    if (checkRadio==="by-drink"){
      ...
      console.log(checkRadio);
      byDrinkID(data.drinks[drinkID].idDrink);
      console.log("ID Sent:" + data.drinks[drinkID].idDrink);
